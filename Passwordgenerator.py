...
def generer_mot_de_passe():
    ...
    try:
        ...
        label_resultat.configure(text="Veuillez entrer un nombre entier pour la longueur du mot de passe.")
        return
    ...
    if len(caracteres_possibles) == 0:
        label_resultat.configure(text="Veuillez sélectionner au moins une option.")
        return
    ...
    if checkbox_imprimer.get():
        try:
            ...
            label_resultat.configure(text="Erreur : impossible d'écrire dans le fichier mot_de_passe.txt.")
...
def creer_fichier():
    try:
        with open("mot_de_passe.txt", "x"):
            label_resultat.configure(text="Le fichier mot_de_passe.txt a été créé.")
    except FileExistsError:
        label_resultat.configure(text="Le fichier mot_de_passe.txt existe déjà.")
    except IOError:
        label_resultat.configure(text="Erreur : impossible de créer le fichier mot_de_passe.txt.")
